...
try:
    ...
    transmittance = np.loadtxt('final_spectrum_topo_5.csv', delimiter=',')
    wavelengths = np.linspace(300, 2500, len(transmittance))
    
    # Load the physical design
    thicknesses = np.loadtxt('optimized_thicknesses_topo_5.txt')
    materials = np.loadtxt('optimized_materials_topo_5.txt', dtype=int)
...
# =========================================================
plt.tight_layout()
plt.savefig('Final_5Material_Topology_Report_5.png', dpi=300, bbox_inches='tight')
plt.show()

print(f"Visualization complete! Stack contains {len(used_materials)} unique materials.")
print("Saved as 'Final_5Material_Topology_Report.png'")