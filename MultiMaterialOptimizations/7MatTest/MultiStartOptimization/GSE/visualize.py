...
try:
    ...
    transmittance = np.loadtxt('CHAMPION_GUMBEL_spectrum.csv', delimiter=',')
    wavelengths = np.linspace(300, 2500, len(transmittance))
    
    # Load the physical design
    thicknesses = np.loadtxt('CHAMPION_GUMBEL_thicknesses.txt')
    materials = np.loadtxt('CHAMPION_GUMBEL_materials.txt', dtype=int)
...
# =========================================================
plt.tight_layout()
plt.savefig('Final_7Material_GUMBEL_Topology_Report.png', dpi=300, bbox_inches='tight')
plt.show()

print(f"Visualization complete! Stack contains {len(used_materials)} unique materials.")
print("Saved as 'Final_7Material_Topology_Report.png'")