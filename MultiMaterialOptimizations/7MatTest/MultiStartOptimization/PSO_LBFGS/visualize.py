...
try:
    ...
    transmittance = np.loadtxt('CHAMPION_spectrum.csv', delimiter=',')
    wavelengths = np.linspace(300, 2500, len(transmittance))
    
    # Load the physical design
    thicknesses = np.loadtxt('CHAMPION_thicknesses.txt')
    materials = np.loadtxt('CHAMPION_materials.txt', dtype=int)
...
# =========================================================
plt.tight_layout()
plt.savefig('Final_7Material_PSO_Topology_Report.png', dpi=300, bbox_inches='tight')
plt.show()
