...
# -----------------------------
# 6) Refactored Training & Evaluation Functions
# -----------------------------
...
def generate_figure_3_5(base_cfg: DesignConfig):
    ...
    Instead of pruning, we just run separate optimizations for each layer count.
    """
    print("\n--- Performing Experiment 3.5 ---")
    
    # Define a single band-stop target
    wl_nm = WAVELENGTHS_NM.cpu().numpy()
    target = torch.ones_like(WAVELENGTHS_NM)
    target[(WAVELENGTHS_NM > 530) & (WAVELENGTHS_NM < 570)] = 0.0 # Match Fig 3.7
    ...
    for layers in layer_counts:
        ...
        cfg = replace(base_cfg, num_layers=layers)
        results = run_optimization(cfg, target, verbose=False)
        results_T[layers] = results['T_ideal'] # Ideal Stage 1 result
        
    fig, axes = plt.subplots(1, 2, figsize=(10, 4))

    # Plot (a): 300 vs 130
    axes[0].plot(wl_nm, results_T[300], 'darkorange', label="300 Layers")
    axes[0].plot(wl_nm, results_T[130], 'b-', label="130 Layers")
    ...
    # Plot (b): 300 vs 200
    axes[1].plot(wl_nm, results_T[300], 'darkorange', label="300 Layers")
    axes[1].plot(wl_nm, results_T[200], 'b-', label="200 Layers")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "fig_3_5.png"), dpi=150)
